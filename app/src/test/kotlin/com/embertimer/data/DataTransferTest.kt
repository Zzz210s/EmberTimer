...
class DataTransferTest {
    private lateinit var src: EmberDatabase
    private lateinit var dst: EmberDatabase

    @Before fun setUp() {
        val ctx = ApplicationProvider.getApplicationContext<Context>()
        src = Room.inMemoryDatabaseBuilder(ctx, EmberDatabase::class.java).allowMainThreadQueries().build()
        dst = Room.inMemoryDatabaseBuilder(ctx, EmberDatabase::class.java).allowMainThreadQueries().build()
    }
